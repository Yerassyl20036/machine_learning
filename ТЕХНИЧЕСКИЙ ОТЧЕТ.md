...
### 2.3 Pipeline предобработки RGB-D данных

Предобработка организована по аналогии с геномным анализом (Quality Control → Alignment → Variant Calling):

**Шаг A. Контроль качества данных (Quality Control)**

- Проверка целостности RGB-D пар
- Валидация диапазона глубины (0.5-10.0 м)
- Анализ покрытия (coverage): % валидных пикселей
- Важные метрики: **resolution, depth_range, %valid_pixels, noise_level**
...
with h5py.File('data/raw/nyu_depth_v2_labeled.mat', 'r') as f:
    ...
    # Quality checks
    valid_depth_ratio = (depths > 0.5).sum() / depths.size
    print(f"Valid depth coverage: {valid_depth_ratio:.2%}")
```

**Проблема:** HDF5-файлы MATLAB хранят массивы в column-major order → требуется transpose.

**Шаг B. Нормализация и выравнивание (Normalization & Alignment)**

- Берём референсные параметры (mean, std из тренировочного split)
- Нормализуем RGB к N(0, 1), depth к [0, 1]
- Выравниваем размерности → получаем стандартизированный формат

```python
from sklearn.preprocessing import StandardScaler

# RGB normalization (референс-независимая)
scaler = StandardScaler()
images_flat = images.reshape(-1, 3)
images_norm = scaler.fit_transform(images_flat).reshape(images.shape)

# Depth normalization (сохраняет метрику)
depths_norm = (depths - depths.min()) / (depths.max() - depths.min())
```

**Шаг C. Разделение на выборки (Train/Test Split)**

- Из полного датасета извлекаем подмножество → получаем train/test splits
- Генотипирование образцов:
  - **train** = образцы для обучения (аналог reference)
  - **test** = образцы для валидации (аналог query)
...
train_size = int(0.6 * 400)  # 60% train
train_idx = indices[:train_size]
test_idx = indices[train_size:]
```

**Шаг D. Экспорт и структурирование**

Структура директорий:
```
data/processed/nyu_depth_v2/
├── rgb/              # RGB изображения (uint8)
├── depth/            # Depth maps (float32)
├── labels/           # Segmentation labels (int16)
├── depth_vis/        # Визуализация (PNG)
└── metadata.csv      # Таблица образцов (аналог VCF)
```

**Шаг E. Аннотация образцов (Sample Annotation)**

- Добавляем метаданные: тип сцены, класс объектов, качество
- Частота класса в датасете (аналог allele frequency)
- Связь с бенчмарками (ScanNet, Matterport3D)

**Пример metadata.csv (аналог VCF-таблицы):**
```
Sample_ID,Scene_Type,Split,Width,Height,Depth_Range,Valid_Pixels,Quality
00000,bedroom,train,640,480,0.5-8.2,98.5%,High
00001,kitchen,train,640,480,0.6-9.1,97.2%,High
00023,bathroom,test,640,480,0.5-5.8,99.1%,High
...
```

**Шаг F. Формирование итогового датасета**

- Кратко и наглядно:
  - Топ-образцы (High quality, complete coverage)
  - Проблемные образцы отдельно (низкое качество, артефакты)
  - Рекомендации по использованию
  - Список классов/распределение
...
**Метрики сегментации (математические формулы):**
...
1. **Mean Intersection over Union (mIoU):**
   ...
   где TP (true positive), FP (false positive), FN (false negative), C = 40 классов

2. **Pixel Accuracy:**
   
   $$Acc = \frac{\sum_{c=1}^{C} TP_c}{H \times W}$$
   
   где H = 480 (height), W = 640 (width)
...
3. **Precision, Recall, F1-Score (macro-averaged):**
   ...
   Macro-averaging (равные веса для всех классов):
...
**Детальная таблица результатов (аналог таблицы вариантов):**

| Sample | Frame | Scene    | GT_Class | Pred_Class | Match | Confidence | Error_Type  | Quality       |
|--------|-------|----------|----------|------------|-------|------------|-------------|---------------|
| 00023  | 23    | bedroom  | wall     | wall       | 1/1   | 0.89       | correct     | High          |
| 00042  | 42    | kitchen  | floor    | floor      | 1/1   | 0.92       | correct     | High          |
| 00087  | 87    | bathroom | ceiling  | wall       | 0/1   | 0.45       | misclass    | Low           |
| 00134  | 134   | living   | chair    | floor      | 0/0   | 0.23       | false_neg   | VUS           |
| 00156  | 156   | office   | table    | background | 0/1   | 0.31       | misclass    | Low           |

**Пояснения к таблице:**

- **GT_Class** — ground truth класс ("референсный" аллель)
- **Pred_Class** — предсказанный класс ("альтернативный" аллель)
- **Match**: 
  - `1/1` = точное совпадение (гомозигота по correct)
  - `0/1` = частичное совпадение/ошибка (гетерозигота)
  - `0/0` = полное несовпадение (гомозигота по error)
- **Confidence** — уверенность модели (аналог depth/quality)
- **Error_Type** — тип ошибки (correct, misclass, false_neg, false_pos)
- **Quality** — клиническая оценка (High, Low, VUS=Uncertain)
...
**Математическая формулировка TSDF:**

**1. Truncated Signed Distance для вокселя v:**

$$d_v = \begin{cases} 
\frac{d_{sensor} - d_{surface}}{d_{trunc}}, & \text{if } |d_{sensor} - d_{surface}| < d_{trunc} \\
\text{sign}(d_{sensor} - d_{surface}), & \text{otherwise}
\end{cases}$$

где:
- $d_{sensor}$ — расстояние от камеры до вокселя (Euclidean)
- $d_{surface}$ — измеренная глубина вдоль луча (от сенсора)
- $d_{trunc}$ — truncation distance = 0.04 м (2× voxel size)

**2. Weighted averaging при мульти-кадровой интеграции:**

$$d_v^{(t+1)} = \frac{w_v^{(t)} \cdot d_v^{(t)} + w_{new} \cdot d_{new}}{w_v^{(t)} + w_{new}}$$

$$w_v^{(t+1)} = \min(w_v^{(t)} + w_{new}, w_{max})$$

где:
- $w_v^{(t)}$ — накопленный вес вокселя на шаге t
- $w_{new}$ — вес нового наблюдения (обычно = 1)
- $w_{max}$ — максимальный вес (предотвращает overflow)

**3. Извлечение поверхности (Marching Cubes):**

Изоповерхность определяется как:
$$S = \{v \in V : d_v = 0\}$$

где V — множество всех вокселей в volume
...
**Метрики реконструкции (математические формулы):**

1. **Root Mean Square Error (RMSE):**
   
   $$RMSE = \sqrt{\frac{1}{N}\sum_{i=1}^{N} (d_i^{pred} - d_i^{true})^2}$$
   
   где N — число валидных пикселей, d — глубина в метрах

2. **Absolute Relative Error (AbsRel):**
   
   $$AbsRel = \frac{1}{N}\sum_{i=1}^{N} \frac{|d_i^{pred} - d_i^{true}|}{d_i^{true}}$$
   
   Нормализация на ground truth для инвариантности к масштабу

3. **Threshold Accuracies (δ-metrics):**
   
   $$\delta_t = \frac{1}{N} \left| \left\{ i : \max\left(\frac{d_i^{pred}}{d_i^{true}}, \frac{d_i^{true}}{d_i^{pred}}\right) < t \right\} \right|$$
   
   где t ∈ {1.25, 1.25², 1.25³} — пороги для оценки качества
   
   **Интерпретация:**
   - δ < 1.25 означает ошибку менее 25%
   - δ < 1.25² означает ошибку менее 56%
   - δ < 1.25³ означает ошибку менее 95%
...
**Детальная таблица реконструкции (аналог клинической таблицы):**

| Sample | Frame | Scene    | GT_Depth | Pred_Depth | Δ(m)  | RMSE  | AbsRel | Error_Type | Quality      |
|--------|-------|----------|----------|------------|-------|-------|--------|------------|---------------|
| 00012  | 12    | bedroom  | 3.45     | 3.47       | +0.02 | 0.020 | 0.58%  | accurate   | Pathogenic    |
| 00029  | 29    | kitchen  | 4.12     | 4.08       | -0.04 | 0.040 | 0.97%  | accurate   | Likely_Path   |
| 00056  | 56    | living   | 2.89     | 2.75       | -0.14 | 0.140 | 4.84%  | underest   | VUS           |
| 00078  | 78    | bathroom | 1.65     | 1.82       | +0.17 | 0.170 | 10.3%  | overest    | Benign        |
| 00091  | 91    | office   | 5.23     | 4.89       | -0.34 | 0.340 | 6.50%  | underest   | VUS           |

**Пояснения к таблице:**

- **GT_Depth** — ground truth глубина в метрах ("референс")
- **Pred_Depth** — предсказанная глубина ("вариант")
- **Δ(m)** — разница (signed error)
- **RMSE** — локальная ошибка для образца
- **AbsRel** — относительная ошибка в процентах
- **Error_Type** — тип ошибки:
  - `accurate` = Δ < 5 см
  - `underest` = недооценка глубины
  - `overest` = переоценка глубины
- **Quality** — клиническая значимость:
  - `Pathogenic` = высокая точность (RMSE < 3 см)
  - `Likely_Path` = хорошая точность (RMSE < 5 см)
  - `VUS` = uncertain (RMSE 5-15 см)
  - `Benign` = допустимая ошибка (RMSE > 15 см)

**Интерпретация:**
- RMSE < 3 см демонстрирует высокую точность (Pathogenic quality)
